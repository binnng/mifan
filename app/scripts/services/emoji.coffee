((window, angular) ->
	...
	# Thanks to https://github.com/coocy/emoji
	...
	Emoji =
	  ...
	  emoji: (text) ->
	    ...
	    supportEmoji = true  if UA.match(/Mac\s+OS/i) and not UA.match(/(Chrome|Firefox)/i)
	    ...
	    if supportEmoji
	      ...
	      pixelRatio = parseFloat(window.devicePixelRatio) or 1
	      Emoji.emojiPath += "2x/"  if pixelRatio > 1.2
	      Emoji.emoji = (text) ->
	        setTimeout (->
	          Emoji.trans text
	          return
	  ...
	  trans: (text) ->
	    ...
	    if text.nodeType
	      el = text
	      fontSize = (el.currentStyle or window.getComputedStyle(el, ""))["fontSize"]
	      
	      #IE浏览器下如果css中的font-size单位不是象素的话，需要转换一下
	      unless /px$/i.test(fontSize)
	        left = el.style.left
	        el.style.left = "1em"
	    ...
	    text = text.replace(Emoji.reg, (code) ->
	      "<img width=" + fontSize + " class=\"emoji\" style=\"vertical-align:middle\" src=\"" + Emoji.emojiPath + Emoji._escapeToUtf32(code) + ".png\">"
	    )
	    el.innerHTML = text  if isElement
	    text
	  ...
	  _convertStringToUnicodeCodePoints: (str) ->
	    surrogate1st = 0
	    unicodeCodes = []
	    i = 0
	    l = str.length
	...
	emoji.directive "p", -> 
		...
		link: (scope, element, attrs) ->
			...
			element = element[0]
			Emoji.emoji element if element && element.innerHTML isnt ""
