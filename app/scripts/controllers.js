...
Mifan.controller("rootCtrl", function($scope, $cookieStore, $http, $timeout, $storage, $emoji, $cacheFactory, $extend, $location, $debug) {
  var API, Ans, Ask, Cache, Comment, Follow, LoveAns, MBill, MDesign, MMenu, Notification, Page, Pagination, Reg, Toast, User, elMwrap, getUserInfo, store;
  API = $scope.API;
  ...
  Ask = {
    init: function() {
      $scope.$on("onGetAskInfo", function(e, askid) {
        return Ask.getInfo(askid);
      });
      return $scope.$on("onGetAskAnswers", function(e, askid) {
        return Ask.getAnswers(askid);
      });
    },
    getInfo: function(askid) {
      var api;
      ...
      return $http.get(api).success(Ask.getInfoCb);
    },
    getInfoCb: function(data) {
    ...
    getAnswers: function(data) {
      ...
      return $http.get(api).success(Ask.getAnswersCb);
    },
